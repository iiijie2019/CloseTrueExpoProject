...
import { Chip, Empty, PageHeader, Reveal, SearchBox, Tap, T, Title, ui } from '@/components/grove/ui';
...
export default function RootsScreen() {
  ...
  return <PagedList items={filtered} resetKey={JSON.stringify([query, kind])} columns={wide ? 2 : 1} header={<><PageHeader title={t('rootStudy')}/><Reveal><Title>{t('rootTitle')}</Title><T style={{ color: c.muted, marginTop: 10, marginBottom: 24 }}>{t('rootSubtitle')}</T><SearchBox value={query} onChange={setQuery} placeholder={t('rootSearch')}/></Reveal>
    ...
    </>} renderItem={({ item: root }) => <View style={{ flex: 1, marginBottom: 14 }}><Tap onPress={() => router.push({ pathname: '/root/[id]', params: { id: root.id } })} style={[ui.card, { backgroundColor: c[root.color], paddingHorizontal: 24, paddingVertical: 12, minHeight: 148 }]}>
    ...
    </Tap></View>} empty={<Empty message={t('noWords')} detail={t('noWordsSub')}/>}/>;
}
