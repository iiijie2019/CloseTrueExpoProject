...
import { AudioButton, posAbbreviation, StatusBadge } from '@/components/grove/word-row';
import { morphemes, wordById, words } from '@/data/lexicon';
import { sentences } from '@/data/sentences';
import { wordTopics, type WordTopic } from '@/data/word-topics';
...
export default function HomeScreen() {
  ...
  return <Page tabs>
    ...
    <View style={{ marginTop: 29 }}><SectionLabel title={label('生活场景', 'Everyday topics')} subtitle={label('认识生活中用得上的单词。', 'Words for everyday moments.')} right={<Tap onPress={() => router.push({ pathname: '/words', params: { showFilters: '1' } })} style={[ui.row, { minHeight: 44, gap: 4 }]}><T style={{ color: c.green, fontSize: 12 }}>{t('allTopics', { count: Object.keys(wordTopics).length })}</T><Icon name="chevron" size={13}/></Tap>}/>
...
const s = StyleSheet.create({
  ...
  search: { width: 44, height: 44, borderRadius: 16, backgroundColor: '#FFFFFFC9', borderWidth: 1, borderColor: '#E4EBDE', justifyContent: 'center', alignItems: 'center' },
