...
export default function RootScreen() {
  ...
  const allWords = familyWords(root.id);
  ...
  const branches = (['noun', 'verb', 'adjective', 'adverb'] as const).filter(pos => visible.some(word => word.pos.includes(pos)));
  const toggle = (pos: PartOfSpeech) => setClosed(old => old.includes(pos) ? old.filter(p => p !== pos) : [...old, pos]);
  return <Page narrow><PageHeader title={t(root.kind)}/>
    <Reveal style={[ui.card, { backgroundColor: c[root.color], padding: 28 }]}>
    ...
    <View style={[ui.chips, { marginBottom: 20 }]}>{(['all', 'known', 'unknown', 'focus'] as const).map(value => <Chip key={value} label={t(value)} selected={filter === value} tint={value === 'focus' ? 'orange' : 'green'} onPress={() => setFilter(value)}/>)}</View>
    {visible.length === 0 ? <Empty message={t('noWords')} detail={t('noWordsSub')}/> : mode === 'list' ? <Reveal key="list" style={ui.card}>{visible.map((word, i) => <WordRow word={word} key={word.id} last={i === visible.length - 1}/>)}</Reveal> : <Reveal key="tree">
      ...
      <View style={{ marginLeft: 24 }}>{branches.map(pos => {
        ...
        const members = visible.filter(word => word.pos.includes(pos));
        return <View key={pos} style={s.branch}>
          ...
          {open && <Reveal style={{ marginLeft: 18, paddingLeft: 16, borderLeftWidth: 1, borderLeftColor: '#DDE5D5', marginTop: 3 }}>{members.map(word => <Tap key={word.id} onPress={() => router.push({ pathname: '/word/[id]', params: { id: word.id } })} style={s.leaf}>
            <View style={{ position: 'absolute', left: -17, top: 33, width: 16, borderTopWidth: 1, borderColor: '#DDE5D5' }}/>
            <View style={{ flex: 1, gap: 5 }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}><T style={{ fontFamily: serif, fontSize: 23, lineHeight: 30 }}>{word.spelling}</T><StatusBadge status={status(word.id)}/></View><T style={{ color: c.muted, fontSize: 12 }} numberOfLines={2}>{local(word.meaning)}</T></View><Icon name="chevron" size={15} color="#9AAB8F"/>
          </Tap>)}</Reveal>}
        </View>;
...
const s = StyleSheet.create({
  ...
  branchHeading: { minHeight: 52, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12, backgroundColor: '#EDF3E8', borderRadius: 15, paddingHorizontal: 13, paddingVertical: 9 },
  posBadge: { width: 36, height: 31, borderRadius: 9, backgroundColor: '#DFEAD7', alignItems: 'center', justifyContent: 'center' },
  leaf: { flexDirection: 'row', alignItems: 'center', gap: 10, borderRadius: 15, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#ECF0E5', padding: 16, marginTop: 11 },
});
