...
import { AudioButton, StatusBadge, posAbbreviation } from '@/components/grove/word-row';
...
export default function WordScreen() {
  ...
  return <Page narrow><PageHeader title={t('discover')}/>
    ...
    {word.forms.length > 0 && <Reveal style={{ marginTop: 28 }}><SectionLabel title={t('forms')}/><View style={ui.chips}>{word.forms.map(form => <Tap key={form} accessibilityLabel={`${t('listen')} ${form}`} onPress={() => void play(form)} style={{ backgroundColor: '#EEF0E8', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 11 }}><T style={{ fontFamily: serif, fontSize: 17, color: '#768064' }}>{form}</T></Tap>)}</View></Reveal>}
    {word.morphemes.length > 0 && <Reveal style={{ marginTop: 30 }}><SectionLabel title={t('connections')}/>{word.morphemes.map(rootId => {
    ...
    <Tap onPress={() => void Linking.openURL(`https://morphemelexicon.com/#/w/${encodeURIComponent(word.spelling)}`).catch(() => notify(t('sourceUnavailable')))} style={{ flexDirection: 'row', gap: 7, alignItems: 'center', paddingVertical: 20 }}><T style={{ fontSize: 12, color: '#799069' }}>{t('source')}</T><Icon name="external" size={13} color="#799069"/></Tap>
  </Page>;
}
